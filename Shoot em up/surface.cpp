...
void Sprite::DrawRotated(int x, int y, Surface* target, double rot)
{
	rot *= (PI/180);
	//create separate surface with sufficient size
	int _size = max(m_Width,m_Height);
	Surface* _temp = new Surface(2*_size,2*_size);
	//setup
	double dsin = cos(rot);
	double dcos = sin(rot);
	Pixel* im = GetBuffer() + m_CurrentFrame*m_Width;
	Pixel* scr = _temp->GetBuffer();
	//apoint a skip color (top left pixel)
	int skip = *(im);
	_temp->Clear(skip);
	//draw rotated on spare surface
	for (int xx = -m_Width/2; xx < m_Width/2; xx ++)
		for (int yy = -m_Height/2; yy < m_Height/2; yy ++)
		{
			int color = *(im + (xx+m_Width/2) + (yy+m_Height/2)*m_Width);
			int drawx = (int)(xx*dsin - yy*dcos + _size);
			int drawy = (int)(xx*dcos + yy*dsin + _size);
			*(scr + drawx + drawy*(2*_size)) = color;
		}
	//copy new surface to screen (or whatever target surface)
	im = scr;
	scr = target->GetBuffer();
	for (int xx = 1; xx < 2*(_size-1); xx ++)
		for (int yy = 1; yy < 2*(_size-1); yy ++)
		{
			int color = *(im + xx + yy*2*_size);
			if (color == skip)
			{
				int _c1 = *(im + xx+1 + yy*2*_size);
				int _c2 = *(im + xx-1 + yy*2*_size);
				if (_c1 && _c2)
					color = _c1;
				else
				{
					int _c1 = *(im + xx + (yy+1)*2*_size);
					int _c2 = *(im + xx + (yy-1)*2*_size);
					if (_c1 && _c2)
						color = _c1;
				}
			}
			if (color == skip) continue;
			int drawx = x + (xx - _size);
			int drawy = y + (yy - _size);
			if (drawx >= target->GetWidth() || drawx < 0) continue;
			if (drawy >= target->GetHeight() || drawy < 0) continue;
			*(scr + drawx + drawy*target->GetWidth()) = color;
		}
	//delete spare surface
	delete _temp;
}
